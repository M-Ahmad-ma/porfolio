...
import { X, ChevronLeft, ChevronRight, Database, Code2, BookOpen, Layers, ArrowRight, Plug, Maximize2, Minimize2, MonitorPlay, ExternalLink, Loader2 } from 'lucide-react'
                ...
                <motion.button
                  ...
                  className={`flex items-center gap-1.5 px-3 py-1.5 text-sm rounded-lg transition-colors ${
                    showPreview 
                      ...
                      : 'bg-tokyo-bg border border-tokyo-border text-tokyo-muted hover:text-tokyo-blue hover:border-tokyo-blue/50'
                  }`}
...
>
            ...
            {/* Live Preview */}
            {showPreview && project.livePreview && (
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <h3 className="flex items-center gap-2 text-lg font-semibold text-tokyo-blue">
                    <MonitorPlay size={18} />
                    Live Preview
                  </h3>
                  <div className="flex items-center gap-2">
                    <a
                      ...
                      className="flex items-center gap-1.5 px-2 py-1 text-xs text-tokyo-muted hover:text-tokyo-blue transition-colors"
                    >
                ...
                <div className="relative w-full h-[600px] bg-tokyo-bg border border-tokyo-border rounded-lg overflow-hidden">
                  {iframeLoading && (
                    <div className="absolute inset-0 flex items-center justify-center bg-tokyo-surface">
                      <Loader2 className="animate-spin text-tokyo-blue" size={32} />
                    </div>
                  )}
                  {iframeError && (
                    <div className="absolute inset-0 flex flex-col items-center justify-center gap-3 bg-tokyo-surface">
                      <p className="text-tokyo-muted">Service unavailable</p>
                      <a
                        href={project.livePreview}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="flex items-center gap-1.5 px-3 py-1.5 text-sm bg-tokyo-blue text-tokyo-bg rounded-lg hover:bg-tokyo-blue/80 transition-colors"
                      >
                  ...
                  <iframe
                    ...
                    className={`w-full h-full ${iframeLoading || iframeError ? 'invisible' : 'visible'}`}
